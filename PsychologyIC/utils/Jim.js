const JMessage = require('jmessage-wxapplet-sdk-1.4.0.min.js'),
    setinfo = getApp().setinfo, jim = new JMessage(), api = require('api.js'), ui = require('Interface.js'),
    ...
    jim.init({
        ...
        switch (data.code) {
            case 0:
                login()
                break;
            default: ui.showToast(showErrorCode(data.code),1); break;
        }
...
function getConversation(that) {
    jim.getConversation().onSuccess(function (e) {
        ...
        var data = e.conversations, Off_lineMsg = allData.Off_lineMsg, iconArr = []
        allData.messageList = []
        for (let i = 0; i < data.length; i++) {
            var url = api.rootUrl + 'expert/' + data[i].username.substring(1, data[i].username.length) + '?token=' + allData.token
            GetIcon(url).then((res) => {
                iconArr[[data[i].username]] = res.data.data.Icon
                allData.messageList.unshift({
...
function onMsgReceive() {
  jim.onMsgReceive(function (data) {
    ...
    wx.vibrateLong()
    if (that.getTabBar().data.selected != 2) {
      allData.msgNum++
    }
...
function login() {
    jim.login({
        username: setinfo.UserCode + allData.userID,
        password: setinfo.UserPwd
    }).onSuccess(function (data) {
        switch (data.code) {
            case 0:
                ...
                onSyncConversation()
                break;
            default: ui.showToast(showErrorCode(data.code),1)
                break;
        }
    }).onFail(function (data) {
        allData.isJim = false
        ui.showToast(showErrorCode(data.code),1)
    });
}
/**
 * 发送文本信息
...
function sendSingleMsg(res) {
    // 发送消息
    jim.sendSingleMsg(res).onSuccess((data, msg)=> {
